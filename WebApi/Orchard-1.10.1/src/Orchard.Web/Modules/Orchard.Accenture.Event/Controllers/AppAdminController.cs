...
namespace Orchard.Accenture.Event.Controllers
{
    ...
    public class AppAdminController : Controller {
        ...
        {
            if (!_orchardServices.Authorizer.Authorize(Permissions.ManageApp, T("Not allowed to manage App")))
                return new HttpUnauthorizedResult();

            var pager = new Pager(_siteService.GetSiteSettings(), pagerParameters);
            var query = _contentManager.Query(VersionOptions.Latest, "App");
            ...
            //pageOfContentItems.Clear();

            //var contentItems = query.Slice(pager.GetStartIndex(), pager.PageSize).ToList();
            //foreach (var item in contentItems)
            //{
            //    var contentPermissions = item.As<ContentPermissionsPart>().ViewContent.Split(',');
            //    if (_orchardServices.WorkContext.CurrentUser != null)
            //    {
            //        var roles = (_orchardServices.WorkContext.CurrentUser.ContentItem).As<UserRolesPart>().Roles;
            //        bool hasMatch = roles.Select(x => x)
            //                  .Intersect(contentPermissions)
            //                  .Any();
            //        if (hasMatch || roles.Count == 0)
            //        {
            //            pageOfContentItems.Add(item);
            //        }
            //    }
            //}

            var pagerShape = Shape.Pager(pager).TotalItemCount(pageOfContentItems.Count());
