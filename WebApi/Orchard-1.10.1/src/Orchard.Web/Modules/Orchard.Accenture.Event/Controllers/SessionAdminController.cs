...
namespace Orchard.Accenture.Event.Controllers
{
    ...
    public class SessionAdminController : Controller
    {
        ...
        {
            if (!_orchardServices.Authorizer.Authorize(Permissions.ManageSession, T("Not allowed to manage Session")))
                return new HttpUnauthorizedResult();

            var pager = new Pager(_siteService.GetSiteSettings(), pagerParameters);
            //var query = _contentManager.Query<SessionPart, SessionPartRecord>(VersionOptions.Latest);
            var query = _contentManager.Query(VersionOptions.Latest, "Session");
